 ...
 * <ul>
 * <li>Consume and ignore any whitespace characters (see {@link parsiii.tokenizer.Char#isWhitepace()}</li>
 ...
 * (see {@link parsiii.tokenizer.Char#isDigit()}. Also if the current character is a '-' and the next is a digit, we try to read
 * a number.</li>
 * <li>If the current character is a letter, parse an ID (see {@link parsiii.tokenizer.Char#isLetter()}. Once this is complete, check if
 ...
 * ({@link #isIdentifierChar(parsiii.tokenizer.Char)} are consumed and returned as SPECIAL_ID</li>
...
public class Tokenizer extends Lookahead<Token> {
     ...
     */
    protected parsiii.tokenizer.LookaheadReader input;
    /*
    ...
    @Override
    public void setProblemCollector(List<parsiii.tokenizer.ParseError> problemCollector) {
    ...
    protected Token fetch() {
        ...
        problemCollector.add(parsiii.tokenizer.ParseError.error(input.current(),
    ...
    protected void skipBlockComment() {
        ...
        problemCollector.add(parsiii.tokenizer.ParseError.error(input.current(), "Premature end of block comment"));
    }
    ...
    protected Token fetchString() {
        ...
        while (!input.current().isNewLine() && !input.current().is(separator) && !input.current().isEndOfInput()) {
            if (escapeChar != '\0' && input.current().is(escapeChar)) {
                ...
                if (!handleStringEscape(separator, escapeChar, result)) {
                    problemCollector.add(parsiii.tokenizer.ParseError.error(input.next(),
        ...
        if (input.current().is(separator)) {
            ...
            problemCollector.add(parsiii.tokenizer.ParseError.error(input.current(), "Premature end of string constant"));
        }
        return result;
    }
     ...
     */
    protected boolean isIdentifierChar(parsiii.tokenizer.Char current) {
        ...
        while (input.current().isDigit() || input.current().is(decimalSeparator) || (input.current()
            ...
            if (input.current().is(groupingSeparator)) {
                ...
                if (result.is(Token.TokenType.DECIMAL)) {
                    problemCollector.add(parsiii.tokenizer.ParseError.error(input.current(), "Unexpected decimal separators"));
                } else {
     ...
     * errors at once.
     *
     * @param pos        the position of the error. Note that {@link Token} implements {@link parsiii.tokenizer.Position}. Therefore the
     ...
     */
    public void addError(parsiii.tokenizer.Position pos, String message, Object... parameters) {
        getProblemCollector().add(parsiii.tokenizer.ParseError.error(pos, String.format(message, parameters)));
    }
     ...
     * work.
     *
     * @param pos        the position of the warning. Note that {@link Token} implements {@link parsiii.tokenizer.Position}.
    ...
    public void addWarning(Position pos, String message, Object... parameters) {
        getProblemCollector().add(parsiii.tokenizer.ParseError.warning(pos, String.format(message, parameters)));
    }
    ...
    /**
     * Throws a {@link parsiii.tokenizer.ParseException} if an error or warning occurred while parsing the input
     *
     * @throws parsiii.tokenizer.ParseException if an error or warning occurred while parsing.
     */
    public void throwOnErrorOrWarning() throws parsiii.tokenizer.ParseException {
        if (!getProblemCollector().isEmpty()) {
            throw parsiii.tokenizer.ParseException.create(getProblemCollector());
        }
    }

    /**
     * Throws a {@link parsiii.tokenizer.ParseException} if an error occurred while parsing the input.
     * <p>
     * All warnings which occurred will be ignored.
     *
     * @throws parsiii.tokenizer.ParseException if an error occurred while parsing.
     */
    public void throwOnError() throws parsiii.tokenizer.ParseException {
        for (parsiii.tokenizer.ParseError e : getProblemCollector()) {
