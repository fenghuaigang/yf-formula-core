...
public class Functions {
     ...
     * 结果精度值
     */
    public static final Function ROUND_FORMULA = new RoundFormulaFunction();
    /**
	...
	public static final Function COUNT = new MultiFunction() {

		@Override
		protected BigDecimal eval(BigDecimal[] params) {
			return new BigDecimal(params!=null?params.length:0);
		}
...
//            return 3;
        ...
        public double eval(List<Expression> args) {
        	if(args.size()==0){
        		...
        		return args.get(0).evaluate2().doubleValue();
        	}
            double check = args.get(0).evaluate2().doubleValue();
            if (Double.isNaN(check)) {
                return check;
            }
            if (Math.abs(check) > 0) {
                return args.get(1).evaluate2().doubleValue();
            } else {
                return args.get(2).evaluate2().doubleValue();
            }
        }
		...
		public BigDecimal eval2(List<Expression> args) {
			return new BigDecimal(eval(args)+"");
		}
    }
}
