...
import parsiii.tokenizer.Token;
import parsiii.tokenizer.Tokenizer;
 ...
 * occur, a {@link parsiii.tokenizer.ParseException} will be thrown. The parser tries to continue as long a possible to provide good
...
public class Parser {
    ...
    static {
        ...
        Map<String,Function> maps = new HashMap<>();
        maps.put("sin", Functions.SIN);
        maps.put("cos", Functions.COS);
        maps.put("tan", Functions.TAN);
        maps.put("sinh", Functions.SINH);
        maps.put("cosh", Functions.COSH);
        maps.put("tanh", Functions.TANH);
        maps.put("asin", Functions.ASIN);
        maps.put("acos", Functions.ACOS);
        maps.put("atan", Functions.ATAN);
        maps.put("atan2", Functions.ATAN2);
        maps.put("deg", Functions.DEG);
        maps.put("rad", Functions.RAD);
        maps.put("abs", Functions.ABS);
//        maps.put("round", Functions.ROUND);
        maps.put("round", Functions.ROUND_FORMULA);
        maps.put("ceil", Functions.CEIL);
        maps.put("floor", Functions.FLOOR);
        maps.put("exp", Functions.EXP);
        maps.put("ln", Functions.LN);
        maps.put("log", Functions.LOG);
        maps.put("sqrt", Functions.SQRT);
        maps.put("pow", Functions.POW);
        maps.put("min", Functions.MIN);
        maps.put("max", Functions.MAX);
        maps.put("rnd", Functions.RND);
        maps.put("sign", Functions.SIGN);
        maps.put("if", Functions.IF);
        maps.put("sum",Functions.SUM);
        maps.put("count",Functions.COUNT);
        maps.forEach((method,func)->{
            registerFunction(method.toLowerCase(),func);
        });
    ...
    public static void registerFunction(String name, Function function) {
        ...
        functionTable.put(name.toLowerCase(), function);
        functionTable.put(name.toUpperCase(), function);
    }
     ...
     */
    public static Expression parse(String input) throws parsiii.tokenizer.ParseException {
     ...
     * @throws parsiii.tokenizer.ParseException if the expression contains one or more errors
     */
    public static Expression parse(Reader input) throws parsiii.tokenizer.ParseException {
     ...
     * @throws parsiii.tokenizer.ParseException if the expression contains one or more errors
     */
    public static Expression parse(Reader input, Scope scope) throws parsiii.tokenizer.ParseException {
     ...
     * @throws parsiii.tokenizer.ParseException if the expression contains one or more errors
     */
    protected Expression parse() throws parsiii.tokenizer.ParseException {
        Expression result = expression().simplify();
        if (tokenizer.current().isNotEnd()) {
            parsiii.tokenizer.Token token = tokenizer.consume();
            errors.add(parsiii.tokenizer.ParseError.error(token,
    ...
    protected Expression atom() {
        if (tokenizer.current().isSymbol("-")) {
            tokenizer.consume();
            BinaryOperation result = new BinaryOperation(BinaryOperation.Op.SUBTRACT, new parsiii.eval.Constant(new BigDecimal("0")), atom());
            result.seal();
            return result;
        }
        ...
        if (tokenizer.current().isSymbol("(")) {
            ...
            expect(parsiii.tokenizer.Token.TokenType.SYMBOL, ")");
            return result;
        }
        if (tokenizer.current().isSymbol("|")) {
            tokenizer.consume();
        ...
        if (tokenizer.current().isIdentifier()) {
            ...
            parsiii.tokenizer.Token variableName = tokenizer.consume();
            try {
                return new VariableReference(scope.getVariable(variableName.getContents()));
            } catch (IllegalArgumentException e) {
                errors.add(parsiii.tokenizer.ParseError.error(variableName,
                                            String.format("Unknown variable: '%s'", variableName.getContents())));
                return new parsiii.eval.Constant(new BigDecimal("0"));
            }
        }
        return literalAtom();
    }
...
//            double value = Double.parseDouble(tokenizer.consume().getContents());
            if (tokenizer.current().is(parsiii.tokenizer.Token.TokenType.ID)) {
                String quantifier = tokenizer.current().getContents().intern();
                if ("n" == quantifier) {
//                    value /= 1000000000d;
                	value = value.divide(new BigDecimal("1000000000"),12,BigDecimal.ROUND_HALF_UP);
                    tokenizer.consume();
                } else if ("u" == quantifier) {
//                    value /= 1000000d;
                	value = value.divide(new BigDecimal("1000000"),12,BigDecimal.ROUND_HALF_UP);
                    tokenizer.consume();
                } else if ("m" == quantifier) {
//                    value /= 1000d;
                	value = value.divide(new BigDecimal("1000"),12,BigDecimal.ROUND_HALF_UP);
                    tokenizer.consume();
...
//                    value *= 1000000000d;
            ...
            return new parsiii.eval.Constant(value);
        }
        parsiii.tokenizer.Token token = tokenizer.consume();
        errors.add(parsiii.tokenizer.ParseError.error(token,
                                    String.format("Unexpected token: '%s'. Expected an expression.",
                                                  token.getSource())));
        return parsiii.eval.Constant.EMPTY;
    }
    ...
    protected Expression functionCall() {
        FunctionCall call = new FunctionCall();
        parsiii.tokenizer.Token funToken = tokenizer.consume();
        Function fun = functionTable.get(funToken.getContents());
        if (fun == null) {
            errors.add(parsiii.tokenizer.ParseError.error(funToken, String.format("Unknown function: '%s'", funToken.getContents())));
        }
        ...
        if (call.getParameters().size() != fun.getNumberOfArguments() && fun.getNumberOfArguments() >= 0) {
            errors.add(parsiii.tokenizer.ParseError.error(funToken,
                                        String.format(
